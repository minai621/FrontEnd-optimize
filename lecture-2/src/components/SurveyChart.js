...
const SurveyChart = () => {
  ...
  useEffect(() => {
    Promise.all([
      fetch(
        'https://minai621-special-invention-x44p94wpxj92pxpq-3001.preview.app.github.dev/answer'
      ).then((res) => res.json()),
      fetch(
        'https://minai621-special-invention-x44p94wpxj92pxpq-3001.preview.app.github.dev/survey'
      ).then((res) => res.json()),
    ]).then(([answer, survey]) => {
